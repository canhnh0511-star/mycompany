...
> **Superseded một phần bởi ADR-0021** (Scan Session/Batch model): draft row giờ luôn thuộc 1
> `ScanImage`/`ScanBatch`, và `confirmed` đổi tên thành `approved`. Nguyên tắc "ghi draft ngay,
> không giữ state tạm ở client" ở ADR này vẫn giữ nguyên hiệu lực — chỉ đổi cách nhóm nhiều ảnh
> thành 1 phiên. Nội dung dưới đây giữ nguyên làm bối cảnh lịch sử.
